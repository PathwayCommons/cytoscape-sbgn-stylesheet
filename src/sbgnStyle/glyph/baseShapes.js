...
const baseShapes = {
  ...
  circle (cx, cy, r, styleMap) {
    return `<circle cx='${cx}' cy='${cy}' r='${r}' style='${styleMap2Str(styleMap)}' />`;
  },
  ...
  concaveHexagon (width, height, styleMap = {}) {
    return `
    <polygon style='${styleMap2Str(styleMap)}'
  ...
  ellipse (cx, cy, rx, ry, styleMap = {}) {
    return `
      <ellipse cx='${cx}' cy='${cy}' rx='${rx}' ry='${ry}' style='${styleMap2Str(styleMap)}' />
    `;
  },
  ...
  line (x1, y1, x2, y2, styleMap = {}) {
    return `<line x1='${x1}' y1='${y1}' x2='${x2}' y2='${y2}' style='${styleMap2Str(styleMap)}' />`;
  },
  ...
  roundRectangle (x, y, width, height, styleMap = {}) {
    return baseRectangle(x, y, width, height, .3*height, .3*height, .3*height, .3*height, styleMap);
  },
  ...
  text (t, x, y, textAnchor, styleMap = {}) {
    return `<text x='${x}' y='${y}' textAnchor='${textAnchor}' style='${styleMap2Str(styleMap)}'>${t}</text>`;
  }
